		...
		if (bullets[i].from_player) {
			for(int e = enemies.size() - 1; e >= 0; e--) {
				if (ofDist(bullets[i].pos.x, bullets[i].pos.y, enemies[e].pos.x, enemies[e].pos.y) < (enemies[e].width + bullets[i].width)/2) {
                    ...
                    float exp_pos_x = bullets[i].pos.x;
                    float exp_pos_y = bullets[i].pos.y;
                    
					bullets.erase(bullets.begin()+i);
					score += 10;
                    Explosion e;
                    
                    e.setup(exp_pos_x, exp_pos_y, min_exp_radius, max_exp_radius, &exp_color, exp_growth);
...
void ofApp::draw_explosions() {
    for (int i = 0; i < explosions.size(); i++) {
        explosions[i].draw();
    }
}
void ofApp::update_explosions() {
    for (int i = 0; i < explosions.size(); i++) {
        if (explosions[i].time_to_die()) {
...
void ofApp::keyPressed(int key){
	// openFrameworks keyPressed only spits out events as they happen
	// i.e... we don't get a list of currently pressed keys
	if (game_state == "game") {
		if (key == OF_KEY_LEFT) {
